...
def midpoint(p1, p2):
    ...
    return (p1 + p2) / 2
def find_coordinates_on_line(point_2_1, point_2_2, z_value):
    # Convert points to numpy arrays
    p1 = np.array(point_2_1)
    p2 = np.array(point_2_2)
    
    # Extract coordinates
    x1, y1, z1 = p1
    x2, y2, z2 = p2
    
    # Calculate the parameter t for the specified z_value
    t = (z_value - z1) / (z2 - z1)
    
    # Calculate the x and y coordinates using the parameter t
    x = x1 + t * (x2 - x1)
    y = y1 + t * (y2 - y1)
    
    return x, y

def find_coordinates_on_line_1(point_2_1, point_2_2, x_value):
    # Convert points to numpy arrays
    p1 = np.array(point_2_1)
    p2 = np.array(point_2_2)
    ...
    # Calculate the parameter t for the specified x_value
    t = (x_value - x1) / (x2 - x1)
    
    # Calculate the y and z coordinates using the parameter t
    y = y1 + t * (y2 - y1)
    z = z1 + t * (z2 - z1)
...
print('binarized_matrix.shape',binarized_matrix.shape)
binarized_matrix[:,180:, :] = 0
# binarized_matrix[/,:, :] = 0
...
centroid_points_4 = []
centroid_points_6 = []
for slice_index in range(195,200):
...
z_value = 194
# 指定切片位置
x, y = find_coordinates_on_line_1(point_2_1, point_2_2, z_value)
closest_point_2_6_1 =np.array([z_value,x,y])
x6, y6 = find_coordinates_on_line_1(point_6_1, point_6_2, z_value)
closest_point_2_6_2 =np.array([z_value,x6,y6])

x4, y4 = find_coordinates_on_line_1(point_4_1, point_4_2, z_value)
closest_point_4 =np.array([z_value,x4,y4])
...
ax.plot(line_2_6[:, 0], line_2_6[:, 1], line_2_6[:, 2], c='g', marker='o', label='line_2_6_z')
ax.scatter(closest_point_4[0], closest_point_4[1], closest_point_4[2], c='b', label='closest_point_4_z')
...
allpoint = np.array([closest_point_2_6_1,closest_point_2_6_2,closest_point_4])
