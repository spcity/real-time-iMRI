...
def get_ijk2ras_matrix(spacing, ijk2lps_direction, origin):
    ...
    ijk2ras_origin = np.array([76.21510, 110.5082550, 0.5])#np.array([-90.7666702, 129.663666, 92.7629547])

    return ijk2ras_origin, ijk2ras_matrix

def get_zframe_configration():
    with open('z-frame-20240630\zframe_configration\config.yaml', 'r', encoding='utf-8') as f:
...
def check_fiducial_geometry(centroid_Points):
    ...
    if (cos_value < np.cos(15 * 3.14159 / 360)):
...
def ijk_to_ras(ijk, origin, matrix):
    """像素坐标IJK到RAS的转化
    """
    temp = np.matmul(matrix, ijk)
    ras = temp + origin
    return ras
