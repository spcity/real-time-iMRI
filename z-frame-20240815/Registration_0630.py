...
class Registration:
    def __init__(self, slice_idx, input_slice, input_dim, input_ijk2ras_matrix, input_ijk2ras_origin, zframe_configration,):
    ...
    def localize_frame(self, slice_idx, centroid_points):
        ...
        for i in range(3):
            pz = np.array(centroid_points[start_idx: start_idx + 3])
            # 质心是二维向量，追加一维变为三维向量
        ...
        point1 = utils.ijk_to_ras(point1, origin=self.ijk2ras_origin, matrix=self.ijk2ras_matrix)
        point2 = utils.ijk_to_ras(point2, origin=self.ijk2ras_origin, matrix=self.ijk2ras_matrix)
        point3 = utils.ijk_to_ras(point3, origin=self.ijk2ras_origin, matrix=self.ijk2ras_matrix)
