...
def main(path, Ld):
    ...
    direction = np.array([[-0.0143, 0.0082, 0.9999],
                 [-0.9999, 0.0004, -0.0143],
                 [-0.0005,-1.0000,0.0082]])
    ...
    origin = np.array([-87.3069839, 114.140900, 67.9643402])
...
if __name__ == '__main__':
    ...
    # Data file path
    vol_path = 'z-frame-20240815\Data\\t1_gre_fsp3d_sag_iso0.7mm_ACS_201'
    ...
    #NDI转RAS
    T_ZROB = np.linalg.inv(T_ROBZ)
    T_RASZ = np.linalg.inv(T_ZRAS)
    T = T_RASZ @ T_ZROB
    R = T[0:3,0:3]
    Trans =T[:3,3].reshape(3,1)
    Point_gold = np.array( [-258.85, 89.27, 169.79]).reshape(3,1)
    Pred_point = R @ Point_gold + Trans
    print(Pred_point)
